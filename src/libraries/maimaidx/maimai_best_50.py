...
class DrawBest(object):
    ...
    def draw(self):
        ...
        #playCountInfo = f'旧版本 {self.sdRating} + 当前版本 {self.dxRating} = {self.playerRating}'
        playCountInfo = f'旧版本: {self.sdRating} + 新版本: {self.dxRating}'
        ...
        authorBoardDraw.text((26, 12), '      Powered by\nXYB & Diving Fish\n    Generated by\nCybeRELF ＃2722', 'black', font2)
        self.img.paste(authorBoardImg, (1224, 19), mask=authorBoardImg.split()[3])

        dxImg = Image.open(self.pic_dir + 'UI_RSL_MBase_Parts_01__50.png').convert('RGBA')
        self.img.paste(dxImg, (988, 65), mask=dxImg.split()[3])
        sdImg = Image.open(self.pic_dir + 'UI_RSL_MBase_Parts_02__50.png').convert('RGBA')
