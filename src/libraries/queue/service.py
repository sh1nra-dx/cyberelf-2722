...
def get_all(group_uim=None):
    ...
    try:
        cursor = dbc.cursor()
        query = "SELECT shop.*, region.region_name FROM qun_queue LEFT JOIN region USING (region_id) LEFT JOIN shop USING (region_id) WHERE qun_queue.qun_uim = %s ORDER BY shop_id ASC"
...
def get_one(command, group_uim=None):
    ...
    try:
        cursor = dbc.cursor()
        query = "SELECT shop.*, region.region_name FROM qun_queue LEFT JOIN region USING (region_id) LEFT JOIN shop USING (region_id) WHERE qun_queue.qun_uim = %s AND shop.command = %s LIMIT 1"
...
def add_player(command, number, group_uim=None):
    ...
    try:
        cursor = dbc.cursor()
        select_query = "SELECT shop.* FROM qun_queue LEFT JOIN region USING (region_id) LEFT JOIN shop USING (region_id) WHERE qun_queue.qun_uim = %s AND shop.command = %s LIMIT 1"
...
def del_player(command, number, group_uim=None):
    ...
    try:
        cursor = dbc.cursor()
        select_query = "SELECT shop.*, region.region_name FROM qun_queue LEFT JOIN region USING (region_id) LEFT JOIN shop USING (region_id) WHERE qun_queue.qun_uim = %s AND shop.command = %s LIMIT 1"
        cursor.execute(select_query, (group_uim, command))
        raw_data = cursor.fetchone()
        shop_id = raw_data[0]
        player_count = raw_data[6]
